...
- **Language switching** (`LanguageContext.tsx`): language is derived from the URL; `setLanguage(lang)` navigates to `swapLangPath(...)` (the same view under the other language prefix) and dispatches `popstate` so the open story stays open. On the **bare home root only**, the app client-redirects `/` → `/en/` from the stored/browser preference (preserves the "English for non-German locales" default); story URLs never redirect, so the prerendered German `/` stays German for crawlers.
...
10. Add story id to the `storyIds` array in `src/App.tsx`
