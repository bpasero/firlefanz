...
- `npx tsx scripts/generate-audio.ts <story-id> [lang|all] [voice]` — generate per-page audio MP3 files via OpenAI TTS (`gpt-4o-mini-tts`, speed 1.0); one TTS call per page for reliable sync; story title and page number are passed in the instructions so the model maintains consistent narrator tone across pages; saved as `public/stories/<id>/audio-<lang>-page-N.mp3`; lang defaults to `de`, pass `all` for every available language; voice defaults to `fable` (available: alloy, echo, fable, onyx, nova, shimmer); **skips pages that already exist** — delete existing MP3s first if regenerating after a text change
