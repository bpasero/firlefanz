...
- `npx tsx scripts/generate-pdf.ts <story-id>` — generate downloadable PDF for a story; **A4 portrait** format with full-width 3:2 illustration at top (no cropping), warm paper text panel below; uses Lora serif (18pt) for body text and Playfair Display for titles; includes cover page, dedication page (*Für Madsi von deinem Papi — Zürich, 2026*), story pages, and a final *Ende* page; fonts loaded from `node_modules/@fontsource` (woff format); images are upscaled to **300 DPI** (2551×1701 px — canvas width including bleed) using Sharp Lanczos3 resampling and embedded as **JPEG quality 90** — visually lossless for print, ~8–12 MB per PDF; PDF canvas includes **3mm bleed on all sides** (612×859 pt vs A4 trim 595×842 pt) — artwork extends into bleed zone so cutting variation leaves no white edges; all text and page numbers stay within the trim safe area
