// © 2026 Benjamin Pasero. All rights reserved.
// https://github.com/bpasero/firlefanz

import { useNightMode } from '../context/NightModeContext'
import { useLanguage } from '../context/LanguageContext'
import Tooltip from './Tooltip'

interface Props {
  narrating: boolean
  onToggle: () => void
  bare?: boolean
}

export default function NarrationToggle({ narrating, onToggle, bare = false }: Props) {
  const { nightMode } = useNightMode()
  const { language } = useLanguage()

  const bg = narrating
    ? (nightMode ? 'rgba(180,140,90,0.45)' : 'rgba(217,116,53,0.5)')
    : bare ? 'transparent' : (nightMode ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.4)')
  const hoverBg = narrating
    ? (nightMode ? 'rgba(180,140,90,0.6)' : 'rgba(217,116,53,0.65)')
    : (nightMode ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.55)')

  return (
    <Tooltip label={narrating ? (language === 'en' ? 'Stop narration' : 'Vorlesen stoppen') : (language === 'en' ? 'Read aloud' : 'Vorlesen')}>
    <button
      onClick={onToggle}
      className="w-9 h-9 sm:w-8 sm:h-8 rounded-full flex items-center justify-center transition-colors"
      style={{
        backgroundColor: bg,
        color: nightMode ? '#f1e0c2' : '#7c4a1e',
      }}
      onMouseEnter={(e) => e.currentTarget.style.backgroundColor = hoverBg}
      onMouseLeave={(e) => e.currentTarget.style.backgroundColor = bg}
    >
      {narrating ? (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
          <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/>
          <path d="M14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/>
        </svg>
      ) : (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
          <path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM3 9v6h4l5 5V4L7 9H3z"/>
        </svg>
      )}
    </button>
    </Tooltip>
  )
}
